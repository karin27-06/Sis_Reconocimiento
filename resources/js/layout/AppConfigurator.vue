...
<template>
  <div
    class="config-panel hidden absolute top-[3.25rem] right-0 sm:right-4 w-56 sm:w-64 md:w-72 p-3 sm:p-4 bg-surface-0 dark:bg-surface-900 border border-surface rounded-border origin-top shadow-[0px_3px_5px_rgba(0,0,0,0.02),0px_0px_2px_rgba(0,0,0,0.05),0px_1px_4px_rgba(0,0,0,0.08)] sm:top-[3.25rem]"
  >
    <div class="flex flex-col gap-4">
      <div>
        <span class="text-sm sm:text-base text-muted-color font-semibold">Primary</span>
        <div class="pt-2 flex flex-wrap gap-2 justify-center sm:justify-between">
          <button
            ...
            :class="['border-none w-5 h-5 sm:w-6 sm:h-6 rounded-full p-0 cursor-pointer outline-none outline-offset-1', { 'outline-primary': layoutConfig.primary === primaryColor.name }]"
            :style="{ backgroundColor: `${primaryColor.name === 'noir' ? 'var(--text-color)' : primaryColor.palette['500']}` }"
          ></button>
        </div>
      </div>
      <div>
        <span class="text-sm sm:text-base text-muted-color font-semibold">Surface</span>
        <div class="pt-2 flex flex-wrap gap-2 justify-center sm:justify-between">
          <button
            ...
            :class="[
              'border-none w-5 h-5 sm:w-6 sm:h-6 rounded-full p-0 cursor-pointer outline-none outline-offset-1',
      ...
      <div class="flex flex-col gap-2">
        <span class="text-sm sm:text-base text-muted-color font-semibold">Presets</span>
        <SelectButton v-model="preset" @change="onPresetChange" :options="presetOptions" :allowEmpty="false" class="w-full sm:w-auto"/>
      </div>
      <div class="flex flex-col gap-2">
        <span class="text-sm sm:text-base text-muted-color font-semibold">Menu Mode</span>
        <SelectButton
          v-model="menuMode"
