<template>
    <div class="flex flex-col sm:flex-row flex-wrap items-stretch sm:items-center gap-3 p-2 w-full">
        <!-- Exportar Excel -->
        <a href="/panel/reports/export-excel-employees" download class="w-full sm:w-auto">
        ...
        <!-- Exportar PDF -->
        <a href="/panel/reports/export-pdf-employees" download class="w-full sm:w-auto">
            <Button variant="outlined" 
                size="small" 
...
const handleFileChange = async (event: Event) => {
    ...
    try {
        ...
        await axios.post('/api/empleados/import-excel', formData, {
            headers: { 'Content-Type': 'multipart/form-data' }
        })

        toast.add({ severity: 'success', summary: 'Importación exitosa', detail: 'Empleados importados correctamente.' })
        emit('import-success')
        target.value = ''
    } catch (error) {
