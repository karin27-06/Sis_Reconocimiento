...
async function openNew() {
    ...
    try {
        // Obtener la última configuración del backend
        const response = await axios.get('/config_alerta/latest');
        if (response.data && response.data.configAlert) {
            configAlerta.value = {
                time: response.data.configAlert.time?.toString() || '',
                amount: response.data.configAlert.amount?.toString() || ''
            };
        }
    } catch (error) {
        console.error("Error al obtener última configuración:", error);
    }
...
async function guardarConfigAlerta() {
    ...
    if (!configAlerta.value.time || !configAlerta.value.amount) return;
...
<Toolbar class="mb-6">
    <template #start>
        <Button label="Configurar Alerta" icon="pi pi-cog" severity="secondary" class="mr-2" @click="openNew" />
    </template>
    <template #end>
        <ToolsAlert/>       
    </template>
        ...
        <div class="grid grid-cols-12 gap-4">
            ...
            <div class="col-span-12 md:col-span-6">
                ...
                <InputText
                    ...
                    class="w-full"
                />
                <small v-if="submitted && !configAlerta.amount" class="text-red-500">La cantidad es obligatoria.</small>
            ...
            <div class="col-span-12 md:col-span-6">
                ...
                <InputText
                    ...
                    class="w-full"
                />
                <small v-if="submitted && !configAlerta.time" class="text-red-500">El tiempo es obligatorio.</small>
